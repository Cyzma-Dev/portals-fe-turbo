  ...
  return (
    <>
      ...
      <div className='flex h-screen w-full'>
        ...
        >
          <div className="flex bg-background flex-col space-y-6 w-full pt-4">
  ...
  return (
    <div className="">
      {item.submenu ? (
        ...
        <Link
          to={item.path}
          className={`flex flex-row space-x-3 items-center p-2 pr-3 rounded-md hover:bg-blueBackground ${item.path === pathname.pathname ? 'bg-blueBackground text-blue' : ''
            }`}
        >
          {item.icon}
          <span className="font-medium text-sm flex">{item.title}</span>
